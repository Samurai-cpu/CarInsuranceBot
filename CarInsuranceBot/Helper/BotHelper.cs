    ...
    public class BotHelper
    {
        ...
        public async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var customerId = message.From!.Id;
        ...
        private Task HandleCommandsAsync(Message message, CancellationToken cancellationToken, long customerId)
        {
            ...
            {
                "/start" => HandleDialogStartAsync(message, cancellationToken, customerId),
        ...
        private Task HandleUserMessageAsync(Message message,Stage userStage , CancellationToken cancellationToken, long customerId)
        {
            ...
            {
                Stage.Greeting when message.Text is not null => HandleDialogStartAsync(message, cancellationToken, customerId),
                ...
                Stage.Finish => HandleFinishStateAsync(message!.Chat.Id, cancellationToken),
        ...
        {
            var openAIRerly = await GetReplyFromOpenAIBasedOnUserMessageAsync(
                """
            ...
            else if (string.Equals(openAIRerly, "no", StringComparison.OrdinalIgnoreCase))
            {
                await Task.WhenAll(_botClient.SendTextMessageAsync(
                    chatId: chatId,
            ...
            else
            {
                await _botClient.SendTextMessageAsync(
                    chatId: chatId,
                    text: await GetReplyFromOpenAIAsync("Tell the user that you didn't understand and ask him to express their thoughts more clearly."),
                    cancellationToken: cancellationToken);
            }
        }
        ...
        private async Task HandlePriceQuotationAsync(string message, long chatId, CancellationToken cancellationToken, long customerId)
        {
            ...
            if (string.Equals(openAIRerly, "yes", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var dummyCarInsurance = await GetDummyInsuranceFromOpenAI(customerData);
                await Task.WhenAll(_botClient.SendTextMessageAsync(
                    chatId: chatId,
                    text: dummyCarInsurance,
                    cancellationToken: cancellationToken),
        ...
        private Task HandleFinishStateAsync(long chatId, CancellationToken cancellationToken)
        {
            return _botClient.SendTextMessageAsync(
                chatId: chatId,
                text: "The conversation with bot is over try to use bot's command if you want to start is again",
                cancellationToken: cancellationToken);
        }
        ...
        private async Task<string> GetReplyFromOpenAIBasedOnUserMessageAsync(string systemMessage, string userMessage)
        {
            try
            {
        ...
        private async Task<string> GetDummyInsuranceFromOpenAI(Customer customerData)
        {
            try
            {
